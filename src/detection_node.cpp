...
{
public:
  DetectionNode(const std::string& model_path, const std::string& label_path)
    : Node("detection_node")
  {
    _img_sub = this->create_subscription<sensor_msgs::msg::Image>(
        "image_raw", 1, std::bind(&DetectionNode::img_cb, this, _1));
...
int main(int argc, char **argv)
{
  ...
  auto pure_args = rclcpp::remove_ros_arguments(argc, argv);
  ...
  std::string model_path = pure_args[1];
  std::string label_path = pure_args[2];
  std::cout << model_path << std::endl;
  std::cout << label_path << std::endl;

  auto node = std::make_shared<DetectionNode>(model_path, label_path);
  rclcpp::spin(node);
  rclcpp::shutdown();
